...
export const sendEmail = async (apiKey: string, to: string, subject: string, html: string) => {
...
export const storeOTP = async (db: D1Database, email: string, purpose: string, code: string) => {
...
export const verifyOTP = async (
	...
	const identifier = `otp-${purpose}:${email}`;
	const row = await db
		.prepare('SELECT id, value, "expiresAt" FROM "verification" WHERE "identifier" = ?')
		.bind(identifier)
	...
	return betterAuth({
		...
		hooks: {
			before: createAuthMiddleware(async (ctx) => {
				...
				const row = await env.DB.prepare("SELECT 1 FROM allowed_students WHERE email = ?")
					.bind(email)
					.first();
				if (!row) {
